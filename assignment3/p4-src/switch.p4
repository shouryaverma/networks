...
control MyIngress(inout headers hdr,
    ...
    table switch_table {
		key = {
			...
			meta.vid: exact;  // Match on VLAN ID from metadata
		}
		actions = {
			forward;
			flood;
    ...
    apply {
        ...
        /**** ADD YOUR CODE HERE ... ****/
		// Check if packet is ARP by looking at saved etherType in metadata
...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        /**** ADD YOUR CODE HERE ... ****/
		// Prune multicast packets going to ingress port
		if (standard_metadata.egress_port == standard_metadata.ingress_port) {
			drop();
		}
		
		// Send copy of ARP packets to controller
		if (standard_metadata.egress_port == CPU_PORT && meta.etherType == ETH_TYPE_ARP) {
			to_controller();
		}
		
		// Apply VLAN filtering if VLAN header exists
		if (meta.vid != 0) {
			vlan_table.apply();
		}
